...
/// </summary>
[TestClass]
public class BothMiddlewareTests
{
    private readonly WebApplicationBuilder builder;
    private HttpClient _httpClient;

    public BothMiddlewareTests()
    {
    ...
    /// </summary>
    [TestMethod]
    public async Task HappyFlows()
    {
        _httpClient.DefaultRequestHeaders.Add("culture", "sv-SE");

        var response = await _httpClient.GetAsync("api/AuthorWithoutFilterController/0");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);

        response = await _httpClient.GetAsync("api/AuthorWithExceptionFilterController/0");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);

        response = await _httpClient.GetAsync("api/AuthorWithExceptionFilterHandleAllController/0");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
    }
    ...
    /// </summary>
    [TestMethod]
    public async Task HappyFlowWithExceptionFilterHandleAll()
    {
        _httpClient.DefaultRequestHeaders.Add("culture", "sv-SE");

        var response = await _httpClient.GetAsync("api/AuthorWithExceptionFilterHandleAll/0");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
    }
